...
router.get("/dashboard/summary", async (_req, res): Promise<void> => {
  ...
  res.json({
    ...
    recentRfqs: recentRfqs.map(r => ({
      ...
      vendorIds: [], items: [], quotationCount: 0, createdAt: new Date(r.createdAt).toISOString(),
    })),
    ...
    recentInvoices: recentInvoices.map(i => ({
      ...
      emailSentAt: i.emailSentAt ? new Date(i.emailSentAt).toISOString() : null, createdAt: new Date(i.createdAt).toISOString(),
    })),
