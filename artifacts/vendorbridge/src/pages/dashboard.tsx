...
export default function Dashboard() {
  ...
  const quickActions = [
    { title: "Create RFQ", icon: FileText, desc: "Draft a new request for quotation", link: "/rfqs/new", bg: "bg-blue-50 hover:bg-blue-100", color: "text-blue-600" },
    { title: "Add Vendor", icon: Users, desc: "Onboard a new supplier", link: "/vendors/new", bg: "bg-green-50 hover:bg-green-100", color: "text-green-600" },
    { title: "Create Invoice", icon: Receipt, desc: "Log a new invoice", link: "/invoices/new", bg: "bg-purple-50 hover:bg-purple-100", color: "text-purple-600" },
    { title: "New PO", icon: ShoppingCart, desc: "Issue a purchase order", link: "/purchase-orders/new", bg: "bg-orange-50 hover:bg-orange-100", color: "text-orange-600" },
  ];
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        {kpis.map((kpi, index) => (
          ...
          >
            <Link href={kpi.link}>
              <Card className="hover:shadow-md transition-all cursor-pointer border-border/50 group relative overflow-hidden h-full">
                <div className={`absolute top-0 right-0 p-3 rounded-bl-2xl transition-colors ${kpi.bg} group-hover:bg-opacity-20`}>
            ...
            {summary.recentRfqs?.length ? (
              <div className="space-y-4">
                {summary.recentRfqs.map((rfq) => (
                  <div key={rfq.id} className="group flex items-center justify-between p-3 rounded-lg border border-border/30 hover:border-border transition-colors hover:bg-muted/20">
             ...
             {summary.recentInvoices?.length ? (
              <div className="space-y-4">
                {summary.recentInvoices.map((inv) => (
                  <div key={inv.id} className="flex items-center justify-between p-3 rounded-lg border border-border/30 hover:border-border transition-colors hover:bg-muted/20">
            ...
            {trendsLoading ? (
              ...
              <ResponsiveContainer width="100%" height={280}>
                <AreaChart data={trends} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                  ...
                  <YAxis tick={{ fontSize: 12 }} tickFormatter={v => `$${(v / 1000).toFixed(0)}k`} axisLine={false} tickLine={false} dx={-10} />
                  <Tooltip 
                    contentStyle={{ borderRadius: '8px', border: '1px solid hsl(var(--border))', backgroundColor: 'hsl(var(--background))' }}
                    formatter={(value: any) => [`$${Number(value).toLocaleString()}`, 'Total Spend']}
                  />
                  <Area type="monotone" dataKey="totalSpend" stroke="#10b981" strokeWidth={3} fillOpacity={1} fill="url(#colorDashboardSpend)" />
                </AreaChart>
              </ResponsiveContainer>
            )}
          </CardContent>
